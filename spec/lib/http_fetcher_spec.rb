...
RSpec.describe Doorkeeper::HttpFetcher do
  ...
  def stub_connections(unreachable: [], seconds_per_attempt: 0, chunks: nil, open_timeouts: [])
    now = 0
    read_timeouts = []
    # An Array gives each successive attempt its own cost; a scalar applies to
    # every attempt.
    per_attempt = seconds_per_attempt.is_a?(Array) ? seconds_per_attempt.dup : nil
    ...
    allow(Net::HTTP).to receive(:new).and_wrap_original do |new_method, *new_args|
      new_method.call(*new_args).tap do |http|
        allow(http).to receive(:start).and_wrap_original do |start_method, *start_args, &block|
          now += per_attempt ? per_attempt.shift.to_i : seconds_per_attempt
          open_timeouts << http.open_timeout
  ...
  describe "#fetch" do
    ...
    it "halves the time left between the two phases of a fallback connection attempt" do
      spent_on_first_address = 6
      open_timeouts = []
      allow(resolver).to receive(:getaddresses).and_return([public_v6_address, public_address])
      stub_connections(
        unreachable: [public_v6_address],
        seconds_per_attempt: [spent_on_first_address, 0],
        open_timeouts: open_timeouts,
      )
      stub_request(:get, url).to_return(status: 200, body: "{}")

      expect(fetcher.fetch(url)).to eq("{}")
      expect(open_timeouts).to eq(
        [
          described_class::OPEN_TIMEOUT,
          (described_class::MAX_TOTAL_TIME - spent_on_first_address) / 2.0,
        ],
      )
    end
    ...
    it "does not hand Net::HTTP a zero open timeout when the budget runs out right before an attempt" do
      allow(resolver).to receive(:getaddresses).and_return([public_v6_address, public_address])
      open_timeouts = []
      stub_connections(unreachable: [public_v6_address], open_timeouts: open_timeouts)
      request_stub = stub_request(:get, url).to_return(status: 200, body: "{}")
      allow(Process).to receive(:clock_gettime).and_return(
        0, 0, described_class::MAX_TOTAL_TIME - 0.001, described_class::MAX_TOTAL_TIME + 1,
      )

      expect { fetcher.fetch(url) }.to raise_error(described_class::FetchError, /could not connect/)
      expect(open_timeouts).to eq([described_class::OPEN_TIMEOUT])
      expect(request_stub).not_to have_been_requested
    end
    ...
    it "raises when reading the body outlives the total time budget" do
      ...
      # The deadline, the connection attempt's budget, the read timeout after
