...
RSpec.describe Doorkeeper::ClientIdMetadata::Document do
  ...
  describe ".parse!" do
    ...
    # The key fetcher reads nothing but https, so such a jwks_uri publishes no
    # key that can ever verify an assertion — same as publishing none at all,
    # and knowable without a single request.
    ["http://client.example.com/jwks.json", "https:jwks.json", "not a uri at all"].each do |value|
