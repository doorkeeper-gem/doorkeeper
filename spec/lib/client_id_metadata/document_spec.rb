# frozen_string_literal: true

require "spec_helper"

RSpec.describe Doorkeeper::ClientIdMetadata::Document do
  let(:url) { "https://client.example.com/oauth-client" }

  def document_body(overrides = {})
    {
      "client_id" => url,
      "client_name" => "Example App",
      "redirect_uris" => ["https://app.example.com/callback"],
      "token_endpoint_auth_method" => "none",
    }.merge(overrides).compact.to_json
  end

  # #document_body drops nil overrides, which is how a client that omits a
  # member serializes. This one keeps them, which is how a client that
  # serializes its whole metadata object does.
  def document_body_with_nulls(overrides = {})
    JSON.parse(document_body).merge(overrides).to_json
  end

  describe ".parse!" do
    it "parses a valid document" do
      document = described_class.parse!(url, document_body)

      expect(document.client_id).to eq(url)
      expect(document.client_name).to eq("Example App")
      expect(document.redirect_uris).to eq(["https://app.example.com/callback"])
      expect(document.token_endpoint_auth_method).to eq("none")
      expect(document).not_to be_confidential
    end

    it "defaults token_endpoint_auth_method to none when absent" do
      body = document_body("token_endpoint_auth_method" => nil)

      document = described_class.parse!(url, body)

      expect(document.token_endpoint_auth_method).to eq("none")
    end

    # An explicit null is how a client that serializes its whole metadata
    # object writes a member it did not set, and every other optional member
    # here already reads it as absent.
    it "defaults token_endpoint_auth_method to none when it is null" do
      body = document_body_with_nulls("token_endpoint_auth_method" => nil)

      document = described_class.parse!(url, body)

      expect(document.token_endpoint_auth_method).to eq("none")
    end

    it "rejects invalid JSON" do
      expect { described_class.parse!(url, "{oops") }
        .to raise_error(described_class::ValidationError, /not valid JSON/)
    end

    it "rejects JSON that is not an object" do
      expect { described_class.parse!(url, "[1,2,3]") }
        .to raise_error(described_class::ValidationError, /not a JSON object/)
    end

    it "rejects a document without a client_id property" do
      expect { described_class.parse!(url, document_body("client_id" => nil)) }
        .to raise_error(described_class::ValidationError, /client_id/)
    end

    it "rejects a client_id that does not match the document URL by simple string comparison" do
      # Even a semantically equivalent URL must not match (RFC 3986 §6.2.1).
      body = document_body("client_id" => "https://CLIENT.example.com/oauth-client")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /does not match/)
    end

    it "rejects a document containing client_secret" do
      body = document_body("client_secret" => "s3cret")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /client_secret/)
    end

    # Same reading of an explicit null as everywhere else here: a client
    # that serializes its whole metadata object — RFC 7591 §3.2.1's
    # registration response has both members — has not used the properties
    # §4.1 forbids, it has only written out their absence.
    it "accepts a document whose forbidden properties are null" do
      body = document_body_with_nulls(
        "client_secret" => nil,
        "client_secret_expires_at" => nil,
      )

      expect(described_class.parse!(url, body).client_id).to eq(url)
    end

    # The name is shown on the consent screen and stored in a string column,
    # so neither an arbitrary JSON value nor an unbounded one may reach it.
    it "rejects a non-string client_name" do
      expect { described_class.parse!(url, document_body("client_name" => { "en" => "App" })) }
        .to raise_error(described_class::ValidationError, /client_name must be a string/)
    end

    it "rejects a client_name longer than the name column can hold" do
      name = "a" * (described_class::MAX_PROPERTY_LENGTH + 1)

      expect { described_class.parse!(url, document_body("client_name" => name)) }
        .to raise_error(described_class::ValidationError, /client_name is longer/)
    end

    # JSON.parse tags a body's bytes as UTF-8 without checking them, so an
    # invalid sequence would otherwise reach the name column and the consent
    # screen that renders it.
    it "rejects a client_name that is not valid UTF-8" do
      body = %({"client_id":"#{url}","client_name":"Ap\xFFp","token_endpoint_auth_method":"none"}).b

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /client_name is not valid UTF-8/)
    end

    # PostgreSQL refuses a NUL in a text column, which would surface as a
    # StatementInvalid the factory does not treat as the document's fault.
    it "rejects a client_name containing a NUL" do
      expect { described_class.parse!(url, document_body("client_name" => "App\u0000")) }
        .to raise_error(described_class::ValidationError, /client_name is not valid UTF-8 or contains a NUL/)
    end

    # The consent screen reads this value out to the end user, and nothing
    # about it is verified: a line break forges a second line of that
    # sentence, and U+202E draws what follows it in reverse, rendering
    # "exe.gnp" as "png.exe".
    it "rejects a client_name containing a line break" do
      expect { described_class.parse!(url, document_body("client_name" => "App\r\nAdministrator")) }
        .to raise_error(described_class::ValidationError, /control or bidirectional/)
    end

    it "rejects a client_name containing a bidirectional override" do
      expect { described_class.parse!(url, document_body("client_name" => "App\u202Eexe.gnp")) }
        .to raise_error(described_class::ValidationError, /control or bidirectional/)
    end

    # The joiners and marks real scripts need are not formatting overrides,
    # and a name using them stays acceptable.
    it "accepts a client_name using a zero-width joiner" do
      name = "App \u{1F468}\u200D\u{1F4BB}"

      expect(described_class.parse!(url, document_body("client_name" => name)).client_name).to eq(name)
    end

    it "accepts a client_name right at the maximum length" do
      name = "a" * described_class::MAX_PROPERTY_LENGTH

      expect(described_class.parse!(url, document_body("client_name" => name)).client_name).to eq(name)
    end

    it "rejects a document containing client_secret_expires_at" do
      body = document_body("client_secret_expires_at" => 0)

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /client_secret_expires_at/)
    end

    %w[client_secret_basic client_secret_post client_secret_jwt].each do |method|
      it "rejects the shared-secret auth method #{method}" do
        body = document_body("token_endpoint_auth_method" => method)

        expect { described_class.parse!(url, body) }
          .to raise_error(described_class::ValidationError, /not allowed/)
      end

      # These names are refused before any strategy is consulted, so nothing a
      # host application declares can let a document select one. A host may
      # register a client_secret_jwt strategy of its own — the metadata endpoint
      # publishes the signing algorithms it declares — and that is about
      # registered clients at the token endpoint; a document naming the method
      # is still refused, whatever the strategy says about itself.
      it "rejects #{method} even when a configured strategy declares it secret-free" do
        Doorkeeper::ClientAuthentication.register(
          :house_assertion,
          double(matches_request?: false, authenticate: nil, uses_shared_secret?: false, auth_method_name: method),
        )
        config_is_set(:client_authentication, %i[none house_assertion])
        body = document_body("token_endpoint_auth_method" => method)

        expect { described_class.parse!(url, body) }
          .to raise_error(described_class::ValidationError, /not allowed/)
      ensure
        Doorkeeper::ClientAuthentication.registered_methods.delete(:house_assertion)
      end
    end

    it "rejects a configured method that declares uses_shared_secret?" do
      Doorkeeper::ClientAuthentication.register(
        :hmac_flavoured,
        double(matches_request?: false, authenticate: nil, uses_shared_secret?: true, auth_method_name: "hmac_flavoured"),
      )
      config_is_set(:client_authentication, %i[none hmac_flavoured])
      body = document_body("token_endpoint_auth_method" => "hmac_flavoured")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not allowed/)
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:hmac_flavoured)
    end

    it "rejects a configured method without a declaration whose name suggests a shared secret" do
      Doorkeeper::ClientAuthentication.register(
        :custom_client_secret_scheme,
        double(matches_request?: false, authenticate: nil, auth_method_name: "custom_client_secret_scheme"),
      )
      config_is_set(:client_authentication, %i[none custom_client_secret_scheme])
      body = document_body("token_endpoint_auth_method" => "custom_client_secret_scheme")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not allowed/)
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:custom_client_secret_scheme)
    end

    it "accepts an unluckily named method that declares it uses no shared secret" do
      Doorkeeper::ClientAuthentication.register(
        :client_secretless_auth,
        double(matches_request?: false, authenticate: nil, uses_shared_secret?: false, auth_method_name: "client_secretless_auth"),
      )
      config_is_set(:client_authentication, %i[none client_secretless_auth])
      body = document_body("token_endpoint_auth_method" => "client_secretless_auth")

      document = described_class.parse!(url, body)

      expect(document.token_endpoint_auth_method).to eq("client_secretless_auth")
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:client_secretless_auth)
    end

    # Two configured strategies may declare one IANA name (a host renaming a
    # method while keeping the old registration key working), and Server stamps
    # the declared name of whichever one matched the request — so a document
    # naming it is authenticated by any of them, and every one has to be
    # secret-free before the document may select it.
    it "rejects a name a second configured method declares with a shared secret" do
      Doorkeeper::ClientAuthentication.register(
        :secretless_mtls,
        double(matches_request?: false, authenticate: nil, uses_shared_secret?: false, auth_method_name: "tls_client_auth"),
      )
      Doorkeeper::ClientAuthentication.register(
        :secret_bearing_mtls,
        double(matches_request?: false, authenticate: nil, uses_shared_secret?: true, auth_method_name: "tls_client_auth"),
      )
      config_is_set(:client_authentication, %i[none secretless_mtls secret_bearing_mtls])
      body = document_body("token_endpoint_auth_method" => "tls_client_auth")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not allowed/)
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:secretless_mtls)
      Doorkeeper::ClientAuthentication.registered_methods.delete(:secret_bearing_mtls)
    end

    it "accepts a name every configured method declares without a shared secret" do
      Doorkeeper::ClientAuthentication.register(
        :secretless_mtls,
        double(matches_request?: false, authenticate: nil, uses_shared_secret?: false, auth_method_name: "tls_client_auth"),
      )
      Doorkeeper::ClientAuthentication.register(
        :renamed_mtls,
        double(matches_request?: false, authenticate: nil, uses_shared_secret?: false, auth_method_name: "tls_client_auth"),
      )
      config_is_set(:client_authentication, %i[none secretless_mtls renamed_mtls])
      body = document_body("token_endpoint_auth_method" => "tls_client_auth")

      expect(described_class.parse!(url, body).token_endpoint_auth_method).to eq("tls_client_auth")
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:secretless_mtls)
      Doorkeeper::ClientAuthentication.registered_methods.delete(:renamed_mtls)
    end

    it "rejects a non-string token_endpoint_auth_method" do
      body = document_body("token_endpoint_auth_method" => 42)

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not allowed/)
    end

    it "rejects auth methods this server is not configured to accept" do
      body = document_body("token_endpoint_auth_method" => "tls_client_auth")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not supported/)
    end

    it "accepts the IANA name a strategy declares, not the key it is registered under" do
      Doorkeeper::ClientAuthentication.register(
        :my_mtls,
        double(
          matches_request?: false, authenticate: nil, auth_method_name: "tls_client_auth", uses_shared_secret?: false,
        ),
      )
      config_is_set(:client_authentication, %i[none my_mtls])
      body = document_body("token_endpoint_auth_method" => "tls_client_auth")

      document = described_class.parse!(url, body)

      expect(document.token_endpoint_auth_method).to eq("tls_client_auth")
      expect(document).to be_confidential
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:my_mtls)
    end

    it "rejects the registration key of a strategy declaring a different IANA name" do
      Doorkeeper::ClientAuthentication.register(
        :my_mtls,
        double(
          matches_request?: false, authenticate: nil, auth_method_name: "tls_client_auth", uses_shared_secret?: false,
        ),
      )
      config_is_set(:client_authentication, %i[none my_mtls])
      body = document_body("token_endpoint_auth_method" => "my_mtls")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not supported/)
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:my_mtls)
    end

    # Server stamps the declared name onto the credentials, so a strategy that
    # declares none could never satisfy the check in Client.authenticate: such
    # a document is refused here rather than materializing a client that could
    # never authenticate.
    it "rejects a configured strategy that declares no IANA name" do
      Doorkeeper::ClientAuthentication.register(
        :tls_client_auth,
        double(matches_request?: false, authenticate: nil),
      )
      config_is_set(:client_authentication, %i[none tls_client_auth])
      body = document_body("token_endpoint_auth_method" => "tls_client_auth")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /not supported/)
    ensure
      Doorkeeper::ClientAuthentication.registered_methods.delete(:tls_client_auth)
    end

    context "when a secret-free method is registered and configured" do
      before do
        Doorkeeper::ClientAuthentication.register(
          :tls_client_auth,
          double(
            matches_request?: false, authenticate: nil, auth_method_name: "tls_client_auth", uses_shared_secret?: false,
          ),
        )
        config_is_set(:client_authentication, %i[client_secret_basic none tls_client_auth])
      end

      after do
        Doorkeeper::ClientAuthentication.registered_methods.delete(:tls_client_auth)
      end

      it "accepts that method and marks the client confidential" do
        body = document_body("token_endpoint_auth_method" => "tls_client_auth")

        document = described_class.parse!(url, body)

        expect(document.token_endpoint_auth_method).to eq("tls_client_auth")
        expect(document).to be_confidential
      end
    end

    # Section 4.1 forbids every shared-secret method, not only the ones it
    # names, so a strategy that has not declared itself secret-free — however
    # secret-free its name sounds — cannot be selected by a document.
    context "when a configured method does not declare whether it uses a shared secret" do
      before do
        Doorkeeper::ClientAuthentication.register(
          :hmac_flavoured,
          double(matches_request?: false, authenticate: nil, auth_method_name: "hmac_flavoured"),
        )
        config_is_set(:client_authentication, %i[none hmac_flavoured])
      end

      after do
        Doorkeeper::ClientAuthentication.registered_methods.delete(:hmac_flavoured)
      end

      it "rejects that method" do
        body = document_body("token_endpoint_auth_method" => "hmac_flavoured")

        expect { described_class.parse!(url, body) }
          .to raise_error(described_class::ValidationError, /not allowed/)
      end
    end

    it "rejects redirect_uris that is not an array" do
      body = document_body("redirect_uris" => "https://app.example.com/callback")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /redirect_uris/)
    end

    # RedirectUriValidator splits the joined value, and String#split raises
    # on an invalid byte sequence — an ArgumentError the factory would let
    # out of the endpoint as a 500.
    it "rejects redirect_uris containing a URI that is not valid UTF-8" do
      body = %({"client_id":"#{url}","redirect_uris":["https://app.example.com/cb\xFF"],"token_endpoint_auth_method":"none"}).b

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /redirect_uris contains a URI that is not valid UTF-8/)
    end

    # RFC 7591 §2: one entry, one URI. The column stores them
    # newline-separated and every reader splits on whitespace, so an entry
    # holding two would quietly register both.
    it "rejects a redirect_uris entry holding more than one URI" do
      body = document_body(
        "redirect_uris" => ["https://app.example.com/cb https://other.example.com/cb"],
      )

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /single URI/)
    end

    it "rejects a blank redirect_uris entry" do
      expect { described_class.parse!(url, document_body("redirect_uris" => [" "])) }
        .to raise_error(described_class::ValidationError, /single URI/)
    end

    it "rejects redirect_uris containing non-strings" do
      body = document_body("redirect_uris" => ["https://app.example.com/callback", 7])

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /redirect_uris/)
    end

    it "allows a document without redirect_uris" do
      document = described_class.parse!(url, document_body("redirect_uris" => nil))

      expect(document.redirect_uris).to eq([])
    end

    it "exposes the scope the client restricts itself to" do
      default_scopes_exist :read
      optional_scopes_exist :write

      document = described_class.parse!(url, document_body("scope" => "read write"))

      expect(document.scope).to eq("read write")
    end

    # A client's own scopes replace the server's as the allow-list rather than
    # narrowing them, so a document naming a scope this server never
    # configured would otherwise be issued a token for it.
    it "rejects a scope this authorization server does not configure" do
      default_scopes_exist :read

      expect { described_class.parse!(url, document_body("scope" => "read admin")) }
        .to raise_error(described_class::ValidationError, /does not configure/)
    end

    it "accepts an empty scope" do
      default_scopes_exist :read

      expect(described_class.parse!(url, document_body("scope" => "")).scope).to eq("")
    end

    it "rejects a scope that is not valid UTF-8" do
      default_scopes_exist :read
      body = %({"client_id":"#{url}","token_endpoint_auth_method":"none","scope":"re\xFFad"}).b

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /scope is not valid UTF-8/)
    end

    it "leaves scope nil when the document omits it" do
      expect(described_class.parse!(url, document_body).scope).to be_nil
    end

    it "rejects a non-string scope" do
      expect { described_class.parse!(url, document_body("scope" => %w[read write])) }
        .to raise_error(described_class::ValidationError, /scope must be a string/)
    end

    it "rejects a scope longer than the scopes column can hold" do
      scope = "a" * (described_class::MAX_PROPERTY_LENGTH + 1)

      expect { described_class.parse!(url, document_body("scope" => scope)) }
        .to raise_error(described_class::ValidationError, /scope is longer/)
    end

    it "accepts a well-formed jwks" do
      body = document_body("jwks" => { "keys" => [{ "kty" => "RSA", "n" => "x", "e" => "AQAB" }] })

      document = described_class.parse!(url, body)

      expect(document.jwks["keys"].size).to eq(1)
    end

    it "accepts an empty keys array" do
      document = described_class.parse!(url, document_body("jwks" => { "keys" => [] }))

      expect(document.jwks).to eq("keys" => [])
    end

    it "accepts a string jwks_uri" do
      document = described_class.parse!(url, document_body("jwks_uri" => "https://client.example.com/jwks.json"))

      expect(document.jwks_uri).to eq("https://client.example.com/jwks.json")
    end

    # Without these the malformed value reaches the JWK Set resolver, which
    # indexes into it and raises a TypeError out of the token endpoint.
    [
      ["an array", [{ "kty" => "RSA" }]],
      ["a number", 42],
      ["a string", "not-a-jwks"],
      ["an object without keys", { "kid" => "x" }],
      ["an object whose keys is not an array", { "keys" => { "kid" => "x" } }],
      ["an object whose keys holds non-objects", { "keys" => ["not-a-key"] }],
    ].each do |description, value|
      it "rejects a jwks that is #{description}" do
        expect { described_class.parse!(url, document_body("jwks" => value)) }
          .to raise_error(described_class::ValidationError, /jwks/)
      end
    end

    # Section 4.1: "private key material MUST NOT be included in the Client ID
    # Metadata Document; only public keys ... are permitted". A symmetric key
    # is the shared secret the same section forbids establishing at all.
    [
      ["an RSA private exponent", { "kty" => "RSA", "n" => "x", "e" => "AQAB", "d" => "s3cret" }],
      ["an RSA CRT parameter", { "kty" => "RSA", "n" => "x", "e" => "AQAB", "qi" => "s3cret" }],
      ["an EC private key", { "kty" => "EC", "crv" => "P-256", "x" => "x", "y" => "y", "d" => "s3cret" }],
      ["a symmetric key", { "kty" => "oct", "k" => "s3cret" }],
      ["a symmetric key with no value", { "kty" => "oct" }],
    ].each do |description, key|
      it "rejects a jwks publishing #{description}" do
        expect { described_class.parse!(url, document_body("jwks" => { "keys" => [key] })) }
          .to raise_error(described_class::ValidationError, /public keys only/)
      end
    end

    # The whole document goes, not just the offending key: a client that has
    # published its own private key is not one to hold to the rest of the set.
    it "rejects a jwks whose private key sits beside a public one" do
      body = document_body(
        "jwks" => {
          "keys" => [
            { "kty" => "RSA", "n" => "x", "e" => "AQAB" },
            { "kty" => "RSA", "n" => "y", "e" => "AQAB", "d" => "s3cret" },
          ],
        },
      )

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /public keys only/)
    end

    it "rejects a non-string jwks_uri" do
      expect { described_class.parse!(url, document_body("jwks_uri" => ["https://client.example.com/jwks.json"])) }
        .to raise_error(described_class::ValidationError, /jwks_uri/)
    end

    # RFC 7591 Section 2: the two are mutually exclusive. Preferring one
    # silently would leave the server verifying against a key set the client
    # may not have meant to be authoritative.
    it "rejects a document carrying both jwks and jwks_uri" do
      body = document_body(
        "jwks" => { "keys" => [{ "kty" => "RSA", "n" => "x", "e" => "AQAB" }] },
        "jwks_uri" => "https://client.example.com/jwks.json",
      )

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /must not both be present/)
    end

    # Same reading of an explicit null: `"jwks": null` publishes no key set,
    # so it does not collide with the jwks_uri that does.
    it "accepts a null jwks beside a jwks_uri" do
      body = document_body_with_nulls(
        "jwks" => nil,
        "jwks_uri" => "https://client.example.com/jwks.json",
      )

      document = described_class.parse!(url, body)

      expect(document.jwks).to be_nil
      expect(document.jwks_uri).to eq("https://client.example.com/jwks.json")
    end

    # A private_key_jwt document publishing no keys would materialize a
    # confidential client that can never authenticate at the token endpoint,
    # leaving rows and grants behind for it. No spec text requires the keys
    # (RFC 7591 §2 lists jwks/jwks_uri as optional) — hygiene, limited to
    # private_key_jwt by name.
    it "rejects a private_key_jwt document that publishes neither jwks nor jwks_uri" do
      config_is_set(:client_authentication, %i[none private_key_jwt])
      body = document_body("token_endpoint_auth_method" => "private_key_jwt")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /requires jwks or jwks_uri/)
    end

    # A blank jwks_uri is never fetched, so it publishes no more keys than an
    # absent one and must not satisfy the check either.
    it "rejects a private_key_jwt document whose jwks_uri is blank" do
      config_is_set(:client_authentication, %i[none private_key_jwt])
      body = document_body("token_endpoint_auth_method" => "private_key_jwt", "jwks_uri" => "")

      expect { described_class.parse!(url, body) }
        .to raise_error(described_class::ValidationError, /requires jwks or jwks_uri/)
    end

    it "accepts a private_key_jwt document that publishes a jwks_uri" do
      config_is_set(:client_authentication, %i[none private_key_jwt])
      body = document_body(
        "token_endpoint_auth_method" => "private_key_jwt",
        "jwks_uri" => "https://client.example.com/jwks.json",
      )

      expect(described_class.parse!(url, body).token_endpoint_auth_method).to eq("private_key_jwt")
    end

    # The key fetcher reads nothing but https, from a port a TCP connection
    # could be made to (URI.parse accepts any integer as one), so such a
    # jwks_uri publishes no key that can ever verify an assertion — same as
    # publishing none at all, and knowable without a single request.
    [
      "http://client.example.com/jwks.json",
      "https:jwks.json",
      "https://client.example.com:99999999999999999999/jwks.json",
      "https://client.example.com:0/jwks.json",
      "not a uri at all",
    ].each do |value|
      it "rejects a private_key_jwt document whose jwks_uri is #{value.inspect}" do
        config_is_set(:client_authentication, %i[none private_key_jwt])
        body = document_body("token_endpoint_auth_method" => "private_key_jwt", "jwks_uri" => value)

        expect { described_class.parse!(url, body) }
          .to raise_error(described_class::ValidationError, /jwks_uri must be an https URL/)
      end
    end

    # Other methods bring their own key discovery, so a document naming one of
    # them is not rejected over a jwks_uri it never authenticates with.
    it "accepts a non-private_key_jwt document whose jwks_uri is not https" do
      body = document_body("token_endpoint_auth_method" => "none", "jwks_uri" => "http://client.example.com/jwks.json")

      expect(described_class.parse!(url, body).jwks_uri).to eq("http://client.example.com/jwks.json")
    end
  end
end
