...
RSpec.describe Doorkeeper::OAuth::PreAuthorization do
  ...
  context "when using PKCE params" do
    ...
    context "when force_pkce is enabled" do
      ...
      context "when an extension-registered response type issues no code" do
        around do |example|
        ...
        before do
          ...
          allow(server).to receive(:grant_flows).and_return(%w[id_token_token])
          attributes[:response_type] = "id_token token"
        end
