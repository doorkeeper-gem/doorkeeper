...
RSpec.describe Doorkeeper::OAuth::BaseRequest do
  ...
  describe "#authorize" do
    ...
    context "when invalid" do
      context "with error other than invalid_request" do
        before do
          allow(request).to receive_messages(valid?: false, error: Doorkeeper::Errors::ServerError, state: "hello")
        end
      ...
      context "with invalid_request error" do
        before do
          allow(request).to receive_messages(valid?: false, error: Doorkeeper::Errors::InvalidRequest, state: "hello")
        end
  ...
  describe "#find_or_create_access_token" do
    ...
    context "when a custom_access_token_attribute collides with the dpop thumbprint" do
      ...
      context "without a dpop proof" do
        before { request.dpop_proof = nil }

        it "does not let the custom_access_token_attribute bind the token" do
          result = request.find_or_create_access_token(
            client,
            resource_owner,
            "public",
