...
RSpec.describe "Revoke Token Flow" do
  ...
  context "with authenticated, confidential OAuth 2.0 client/application" do
    ...
    context "when the token was refreshed" do
      ...
      # The previous refresh token is revoked when the access token issued
      # with the next one is first used (`previous_refresh_token`), while the
      # records issued after it along the chain stay live.
      it "revokes every token of the chain when the presented refresh token is already revoked" do
        tokens = chain
        access_protected_resource_with(tokens.second.token)
        expect(tokens.first.reload).to be_revoked

        post revocation_token_endpoint_url,
             params: { token: tokens.first.refresh_token, token_type_hint: "refresh_token" },
             headers: headers
    ...
    context "when the request uses more than one client authentication method" do
      it "rejects the request per RFC 6749 §2.3 and does not revoke the token" do
        post revocation_token_endpoint_url,
             params: {
               client_id: private_client_application.uid,
               client_secret: private_client_application.secret,
             ...
             headers: { "HTTP_AUTHORIZATION" => basic_auth_header_for_client(private_client_application) }
