...
feature 'Revoke Token Flow' do
  ...
  context 'with default parameters' do
    given(:client_application) { FactoryGirl.create :application }
    given(:resource_owner) { User.create!(name: 'John', password: 'sekret') }
    given(:authorization_access_token) do
      FactoryGirl.create(:access_token,
                         application: client_application,
                         resource_owner_id: resource_owner.id,
                         use_refresh_token: true)
    end
    given(:headers) { { 'HTTP_AUTHORIZATION' => "Bearer #{authorization_access_token.token}" } }
    ...
    context 'The access token to revoke is the same than the authorization access token' do
      given(:token_to_revoke) { authorization_access_token }
    ...
    context 'The access token to revoke authorization owner is the same than the authorization access token' do
      given(:other_client_application) { FactoryGirl.create :application }
      given(:token_to_revoke) do
        FactoryGirl.create(:access_token,
    ...
    context 'The access token to revoke app is the same than the authorization access token' do
      given(:other_resource_owner) { User.create!(name: 'Matheo', password: 'pareto') }
      given(:token_to_revoke) do
        FactoryGirl.create(:access_token,
                           application: client_application,
