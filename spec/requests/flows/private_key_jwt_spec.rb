...
feature "private_key_jwt client authentication with Client ID Metadata Documents" do
  ...
  # The jwks_uri is fetched from a host the client names, so its answers get
  # the same treatment as the metadata document's: a rejected client, never an
  # exception out of the token endpoint.
  ...
  scenario "a confidential URL client cannot exchange the code without authentication" do
    code = authorize_and_return_code
