...
RSpec.describe "Authorized applications endpoint", type: :request do
  ...
  # place. To exercise the real API-only hierarchy in process, the controller
  # constants are stubbed with bare ActionController::API subclasses and the
  # real controller sources are re-evaluated into them. The sources are eval'd
  # under a file name of their own rather than `load`ed, so that coverage stays
  # attached to the code the other examples run.
