...
- [#1840] Add a client authentication methods registry (RFC 6749 §2.3). Client authentication is now resolved through pluggable strategies (`client_secret_basic`, `client_secret_post`, `none`) configured via the new `client_authentication` option, and custom methods can be registered with `Doorkeeper::ClientAuthentication.register`.
  - **Breaking change**: the `client_secret_post` and `none` strategies now read client credentials from the request body only (`request.request_parameters`), per RFC 6749 §2.3.1. Clients that sent `client_id`/`client_secret` in the query string of the token endpoint will no longer authenticate and must send them in the request body instead.
  - **Deprecation**: the `client_credentials` option is deprecated. `:from_basic` and `:from_params` are automatically converted to `:client_secret_basic` and `:client_secret_post` (with `:none` appended).
  - **Deprecation**: `Doorkeeper.config.client_credentials_methods` has been renamed to `client_authentication_methods`. The old name remains as a delegating alias (emitting a deprecation warning) for one release and will be removed afterwards.
  - **Upgrade note**: `Doorkeeper::OAuth::Client::Credentials` has moved to `Doorkeeper::ClientAuthentication::Credentials`. Code referencing the old constant must be updated.
