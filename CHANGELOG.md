...
- [#1840] Introduce a pluggable client authentication registry (RFC 6749 §2.3). Client authentication is now resolved through named strategies instead of the hard-coded basic/params lookup, making the supported methods explicit, configurable, and extensible.
  - **New `client_authentication` option**: declare which methods Doorkeeper accepts and the order in which they are tried, e.g. `client_authentication %i[client_secret_basic client_secret_post none]` (the default). The three built-in strategies are `client_secret_basic` (HTTP Basic, RFC 6749 §2.3.1), `client_secret_post` (credentials in the request body) and `none` (public clients with no secret).
  - **Custom methods**: register your own strategy with `Doorkeeper::ClientAuthentication.register(:my_method, MyStrategy)`. A strategy responds to `matches_request?` and `authenticate`, returning a `Doorkeeper::ClientAuthentication::Credentials`. See the [wiki](https://github.com/doorkeeper-gem/doorkeeper/wiki/Changing-how-clients-are-authenticated) for the contract.

  This ships with the following **breaking changes**:
  - Client credentials are no longer read from the query string. The `client_secret_post` and `none` strategies now look at the request body only (`request.request_parameters`), per RFC 6749 §2.3.1 — sending secrets in a URI query is explicitly discouraged by the spec. Clients that passed `client_id`/`client_secret` in the token endpoint query string will stop authenticating and must move them into the request body (or use HTTP Basic).
  - The `none` strategy additionally requires the `Authorization` header to be absent or blank: a request that carries a non-blank one (e.g. a `Bearer` token, or a malformed `Basic` header) is attempting header-based authentication and is no longer silently treated as an unauthenticated public client the way the legacy `from_params` extractor did. Public clients must send their `client_id` in the request body without an `Authorization` header.
  - `Doorkeeper::OAuth::Client::Credentials` has moved to `Doorkeeper::ClientAuthentication::Credentials`. The old constant remains as a deprecated alias for one release (marked with `deprecate_constant`, so Ruby warns on access when deprecation warnings are enabled) so referencing code does not raise `NameError`, but its legacy `.from_request` / `.from_basic` / `.from_params` class methods are gone (extraction now goes through the registry). Update references to the new constant.
  - The `client_authentication` option is honoured exactly as written — both the array form (`client_authentication %i[client_secret_basic client_secret_post none]`) and the varargs form (`client_authentication :client_secret_basic, :client_secret_post`) are accepted, and nothing (in particular `:none`) is appended, so a restrictive configuration is never silently broadened. A configuration that resolves to zero registered methods is now logged as an error at boot instead of silently failing every client authentication.
  ...
  - `Doorkeeper.config.client_credentials_methods` has been renamed to `client_authentication_methods`; the old name remains as a delegating alias that emits a one-shot deprecation warning. The alias returns the legacy method-name symbols (e.g. `:from_basic`, `:from_params`) rather than the internal `Method` objects, so external consumers such as `doorkeeper-openid_connect` (which map those symbols in `token_endpoint_auth_methods_supported`) keep working unchanged for one release.

  **Upgrade notes**:
  - If you rely on query-string client credentials at the token endpoint, switch your clients to the request body or HTTP Basic before upgrading.
...
- [#1841] Reject token requests that use more than one client authentication method (RFC 6749 §2.3: "The client MUST NOT use more than one authentication method in each request"). When a request matches multiple client authentication strategies it is now rejected with an `invalid_request` error instead of silently using the first match.
