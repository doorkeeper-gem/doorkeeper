...
- [#1897] Add experimental support for OAuth Client ID Metadata Documents (draft-ietf-oauth-client-id-metadata-document): with the new `use_client_id_metadata_documents` config option, clients can identify themselves with an `https://` client_id whose metadata Doorkeeper fetches, validates (SSRF-guarded, no redirects, shared-secret auth methods and published key material rejected, only scopes this server configures) and materializes as an application row. Using `private_key_jwt` with such clients requires the server to identify itself through `issuer` or Rails' `default_url_options[:host]`: their assertions are never audience-checked against the request's `Host` header, since a document `client_id` resolves to the same client at every server implementing the draft. Part of [#1875].
- [#1897] Fix: a `private_key_jwt` assertion whose `exp` or `nbf` overflows to `Infinity` (JSON has no such literal, but `1e400` parses as one) is now refused instead of reaching the endpoint as a 500 — the `jwt` gem casts those claims with `to_i`, which raises a `FloatDomainError` that failed verification is not expected to produce.
- [#1897] Fix: a JWK Set carrying private key material no longer verifies a `private_key_jwt` assertion. Verifying needs no private parameter, so a key published with one is a disclosed credential rather than a usable key — symmetric keys were already dropped for the same reason.
- [#1897] Fix: the 10-second ceiling on fetching a `jwks_uri` (or a metadata document) now covers the whole HTTP exchange. `Net::HTTP` applies its timeouts per phase and restarts `read_timeout` on every successful read, so a host answering its status line and headers a byte at a time could hold the connection, and the thread reading it, indefinitely.
- [#1897] The consent screen now shows the host of a Client ID Metadata Document client's `client_id` next to the name its document chose (draft Section 8.5), through a new `doorkeeper.authorizations.new.client_id_host_html` translation. Host applications overriding `authorizations/new` are unaffected unless they adopt the new block.
- [#1897] Fix: the built-in `private_key_jwt` replay guard now accounts for remembered `jti` values per client, so a client flooding it with its own assertions evicts its own entries once the guard is full rather than another client's still-valid replay record.
- [#1897] Fix: `jwks_uri` and metadata document fetches now also refuse hosts resolving to `64:ff9b:1::/48` (local-use IPv4/IPv6 translation), `3fff::/20` (documentation) and `5f00::/16` (SRv6 SIDs), completing the IANA special-purpose address registry coverage.
