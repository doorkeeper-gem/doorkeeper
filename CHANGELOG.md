...
- [#1904] Add opt-in client secret rotation (`enable_secret_rotation`): `Application#rotate_secret!` retains the superseded secret so it keeps authenticating the client until `#clear_old_secret!` ends the grace period, removing the downtime between rotating a secret and redeploying the client. Requires new `old_secret` / `old_secret_created_at` columns — run `rails generate doorkeeper:secret_rotation`. Doorkeeper warns at boot when the companion options are set without the option itself, and the first time the columns are looked for when the migration is missing. `#clear_old_secret!` takes an optional `retained_at:`, so a job that selected rows by an expired `old_secret_created_at` ends the grace period it looked at rather than one a rotation opened in between. Closes [#1675].
