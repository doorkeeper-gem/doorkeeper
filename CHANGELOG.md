...
- [#1897] Add experimental support for OAuth Client ID Metadata Documents (draft-ietf-oauth-client-id-metadata-document): with the new `use_client_id_metadata_documents` config option, clients can identify themselves with an `https://` client_id whose metadata Doorkeeper fetches, validates (SSRF-guarded, no redirects, shared-secret auth methods and private or symmetric key material rejected, only scopes this server configures) and materializes as an application row, and the consent screen and the authorized applications page show that host next to the name the document chose (new `doorkeeper.authorizations.new.client_id_host_html` translation; hosts that copied the authorization views or translations need to port that line), including for rows already materialized when the option is later turned off. The feature requires a host-added `client_id_metadata_materialized_at` datetime column on the applications table (a declared field with the Mongoid extension), which tells the rows it materializes apart from registered applications; see the option's notes in the generated initializer. Document clients are asked for consent on every authorization, a public ("none") document client is refused the `client_credentials` and `password` grants and the introspection endpoint, and the authorization server metadata advertises `client_id_metadata_document_supported`. Using `private_key_jwt` with such clients requires the server to identify itself through `issuer` or Rails' `default_url_options[:host]`, since a document `client_id` resolves to the same client at every server implementing the draft. The shared `private_key_jwt` and remote fetch paths are hardened along the way, which reaches registered `private_key_jwt` clients from 6.0.0.beta2 too: `exp`/`nbf`/`iat` must be finite JSON numbers and `jti` at most 255 characters, JWK Sets carrying private or symmetric key material are refused, `nbf` is always verified and a host-configured global JWT leeway extends the replay guard's memory of a `jti` accordingly, an assertion's audience may name the token endpoint URL built from either `issuer` or Rails' `default_url_options[:host]` when both are configured, the replay guard remembers `jti` values per client (a custom `private_key_jwt_replay_guard` now receives keys shaped `"<length>:<client_id>:<jti>"`, prefixed `url:` for a document client, so entries stored before the upgrade no longer match; the prefix marks which pool the built-in guard accounts an entry in rather than which assertion it is, so single use is decided on the key without it), the keys an assertion was verified against travel with the credentials it produces (`VerifiedCredentials#from_metadata_document?`) so that a client resolved after verification cannot have become the other kind in between, the 10-second fetch ceiling covers the whole HTTP exchange, the 5 KB body cap is backed by a 13 KB socket budget that counts the headers too and a non-UTF-8 body is refused, and four more IANA special-purpose address ranges are blocked. The authorization server metadata now advertises each client authentication method under the IANA name its strategy declares (`auth_method_name`) rather than its registration key — hosts running doorkeeper-openid_connect need its matching change so `/.well-known/openid-configuration` does not disagree — and, where an assertion-based method is enabled, the `token_endpoint_auth_signing_alg_values_supported` RFC 8414 requires alongside it — gathered from the advertised strategies' new `auth_signing_alg_values` declaration, so a host application's own `client_secret_jwt` publishes its algorithms too — and a strategy that does not declare `uses_shared_secret?` is treated as secret-based. The RFC 6749 §2.3 multiple-methods check now counts strategies rather than registration keys, so one strategy registered under several names is a single method instead of a request refused as `invalid_request`. Part of [#1875].
