...
- [#1840] Introduce a pluggable client authentication registry (RFC 6749 §2.3). Client authentication is now resolved through named strategies instead of the hard-coded basic/params lookup, making the supported methods explicit, configurable, and extensible.
  ...
  - `Doorkeeper::OAuth::Client::Credentials` has moved to `Doorkeeper::ClientAuthentication::Credentials`. The old constant remains as a deprecating alias for one release so referencing code does not raise `NameError`, but its legacy `.from_request` / `.from_basic` / `.from_params` class methods are gone (extraction now goes through the registry). Update references to the new constant.
