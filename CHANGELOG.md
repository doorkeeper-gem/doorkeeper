...
- [#1884] Internal: merge `CHANGELOG.md` with git's `union` driver, so two pull requests that each add an entry no longer conflict on the line above "Please add here".
- [#1880] Add experimental support for OAuth Client ID Metadata Documents (draft-ietf-oauth-client-id-metadata-document): with the new `use_client_id_metadata_documents` config option, clients can identify themselves with an `https://` client_id whose metadata Doorkeeper fetches, validates (SSRF-guarded, no redirects, shared-secret auth methods rejected) and materializes as an application row. Part of [#1875].
- [#1880] Add experimental support for OAuth Client ID Metadata Documents (draft-ietf-oauth-client-id-metadata-document): with the new `use_client_id_metadata_documents` config option, clients can identify themselves with an `https://` client_id whose metadata Doorkeeper fetches, validates (SSRF-guarded, no redirects, shared-secret auth methods rejected) and materializes as an application row. Also adds a built-in opt-in `private_key_jwt` client authentication method (RFC 7523 / OIDC Core §9, requires the `jwt` gem) verifying assertions against the client's published keys, which lets these URL clients act as confidential clients. Part of [#1875].
- Please add here
