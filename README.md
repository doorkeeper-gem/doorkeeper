...
`old_secret` and `old_secret_created_at` join the names Doorkeeper adds to
`config.filter_parameters` (`client_secret`, the tokens, and `code` when the
authorization code flow is enabled), which Active Record carries over into
`filter_attributes`, so the retained secret — and the timestamp that says a
rotation is under way — are filtered from logs and from an inspected
application record the way those are. They are filtered whether or not the
option is on: turning it off does not clear a secret an earlier rotation
retained.
...
Called without it, `#clear_old_secret!` ends whatever grace period the row holds, which is what an admin ending one by hand means.
