...
Doorkeeper.configure do
  ...
  # - `private_key_jwt` requires this server to identify itself, through the
  #   `issuer` option above or Rails' `default_url_options[:host]` (see the
  #   client_authentication notes). That matters twice over for document
  #   clients: a document client_id resolves to the same client, and the same
  #   keys, at every server implementing the draft, so an audience taken from
  #   the request's Host header (RFC 7523 Section 3) would make an assertion
  #   sent to one such server replayable at all the others by whoever
  #   received it.
