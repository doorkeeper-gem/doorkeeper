...
Doorkeeper.configure do
  ...
  # scopes, confidential flag) are refreshed from the fetched document,
  # while the grants and tokens already issued to it stay attached to the
  # row - whoever controls that URL inherits them. Audit existing uids
  ...
  #   identify itself, through `issuer` below or Rails'
  ...
  #   refused and a warning is logged at boot. Registered applications are
  #   unaffected.
  ...
  #   `grant_flows` before enabling this. The one exception is
  #   `client_credentials`, which RFC 6749 Section 4.4 reserves for
  #   confidential clients: a document naming "none" is refused that grant,
  #   so hosting a document is not by itself enough to obtain a token.
  # - A document naming "none" does, however, authenticate at the
  #   introspection endpoint the way any public client does, and the
  #   default `allow_token_introspection` lets any authorized client
  #   introspect a token issued to no application. If that is not wanted,
  #   configure `allow_token_introspection` to refuse clients whose uid is
  #   a URL (or public clients altogether).
  ...
  # - Only documents that resolve are memoized, and the memo is keyed by the
  #   client_id exactly as it was sent (the draft requires client_ids to be
