...
module Doorkeeper
  module ClientIdMetadata
    # Fetches a Client ID Metadata Document over HTTPS following the draft's
    # Section 4 rules: redirects are never followed and any status other than
    # 200 OK is an error.
    #
    # SSRF hardening (draft Section 6.5): the host is resolved up front and
    # the request is refused when any resolved address falls into an RFC 6890
    # special-use range (loopback, private-use, link-local, ...). The
    # connection is then pinned to the vetted address via Net::HTTP#ipaddr=
    # so a second, post-check DNS resolution (DNS rebinding) cannot redirect
    # the request; TLS is still negotiated and verified against the original
    # hostname. The draft's exception for authorization servers themselves
    # running on a loopback interface is intentionally not implemented.
    #
    # The response body is bounded (draft Section 6.6) and so is the total
    # time spent reading it: a per-read timeout alone does not stop a server
    # that dribbles bytes out indefinitely.
    #
    # Everything about the response is chosen by whoever hosts the document —
    # which is whoever sent the client_id — so no failure mode here may
    # escape as anything other than a FetchError.
    class HttpFetcher
      OPEN_TIMEOUT = 5
      READ_TIMEOUT = 5

      # Draft Section 6.6 recommends a maximum response size of 5 kilobytes.
      ...
      # Draft Section 4.1: the document is served as JSON, either
      # "application/json" or an "application/<AS-defined>+json" variant. A
      # response declaring anything else is not a metadata document and is
      # refused without being parsed. A response declaring no media type at
      # all is tolerated — the check is there to catch a URL that plainly
      # serves something else, not as a security control, since the body
      # still has to parse and validate as a document.
      JSON_MEDIA_TYPE = %r{\Aapplication/([\w.+-]+\+)?json\z}i

      # RFC 6890 special-purpose IPv4/IPv6 registries, plus multicast ranges
      # (224.0.0.0/4, ff00::/8), which are equally unfit as a metadata origin.
      SPECIAL_USE_RANGES = [
      ...
      def perform_request(uri, address)
        ...
        http.start do |connection|
          # Net::HTTP never follows redirects on its own, satisfying
          # Section 4; a 3xx just fails the status check below.
      ...
      def bounded_body(response, host, deadline)
        ...
        if declared && declared.to_i > MAX_RESPONSE_SIZE
          raise FetchError, "#{host} declares a #{declared} byte metadata document, over the " \
        ...
        response.read_body do |chunk|
          ...
          if body.bytesize > MAX_RESPONSE_SIZE
            raise FetchError, "metadata document from #{host} exceeds #{MAX_RESPONSE_SIZE} bytes"
          elsif monotonic_now > deadline
            raise FetchError, "reading the metadata document from #{host} took too long"
          end
        end
