...
module Doorkeeper
  module ClientIdMetadata
    ...
    module UrlValidator
      ...
      # URI.parse happily returns an arbitrarily large integer for the port,
      # but Net::HTTP formats it into the connection address, where a value
      # too large to be a port raises TypeError rather than any of the
      # transport errors a fetch is prepared for. A port outside the range a
      # TCP port can occupy could never have been connected to anyway. The
      # fetcher's own range, so the two can never disagree on what is
      # connectable.
      PORT_RANGE = HttpFetcher::PORT_RANGE
      ...
      # Read off the raw authority rather than through URI#userinfo, which
      # answers nil for an empty one ("https://@host/path"). Section 3 forbids
      # the component, and the delimiter is the component: without this,
      # https://@host/x and https://host/x are two distinct clients.
      def self.no_userinfo?(url)
        url[%r{\A[A-Za-z][A-Za-z0-9+.-]*://[^/?#]*@}].nil?
      end
      private_class_method :no_userinfo?
