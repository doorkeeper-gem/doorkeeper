...
module Doorkeeper
  module ClientIdMetadata
    ...
    # deliberately implements none of the HTTP caching semantics Section 4.4
    # recommends (that is a follow-up).
    #
    # Only successfully fetched and validated values may be stored: the draft
    # forbids caching error responses, invalid or malformed documents
    # (Section 4.4), which is guaranteed by callers never yielding anything
    # but a validated value.
    class DocumentCache
      DEFAULT_TTL = 60
      MAX_ENTRIES = 500
