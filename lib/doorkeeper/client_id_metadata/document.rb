...
module Doorkeeper
  module ClientIdMetadata
    ...
    class Document
      ...
      # they are, so a malformed sequence reaches the column this value is
      # written to (and the consent screen that renders it). Which adapter
      # raises on the insert differs, so the value is refused here instead.
      def too_long?(value)
      ...
      # neither key representation describes a confidential client that can
      # never authenticate at the token endpoint, yet the authorization
      # endpoint would still materialize it and leave rows and grants behind
      # for it. No spec text requires the keys to be present (RFC 7591 §2
      # lists jwks/jwks_uri as optional), so this is hygiene rather than
      # compliance — and it narrows the case rather than eliminating it: an
      # empty keys array or an unreachable jwks_uri still validates. The
      # check is limited to private_key_jwt by name; other registered
      # methods define their own key discovery.
      def validate_private_key_jwt_keys!(uri, set)
        ...
        return unless set.nil? && uri.blank?
        return unless token_endpoint_auth_method == "private_key_jwt"

        raise ValidationError, "token_endpoint_auth_method private_key_jwt requires jwks or jwks_uri"
      end
    end
  end
end
