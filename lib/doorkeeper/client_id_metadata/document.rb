...
module Doorkeeper
  module ClientIdMetadata
    ...
    class Document
      ...
      # The consent screen is the one place a document's own words are read
      # by the end user, and nothing about them is verified (Section 8.5).
      # A C0 or C1 control puts a line break or a terminal escape in the
      # middle of that sentence, and the bidirectional overrides and
      # isolates reorder what is drawn after them: U+202E renders
      # "exe.gnp" as "png.exe" without changing a byte of it. Neither
      # belongs in a display name, so both are refused rather than escaped;
      # the marks and joiners real scripts need (U+200D, U+200F) are left
      # alone.
      UNSAFE_DISPLAY_CHARACTERS = /[\p{Cc}\u202A-\u202E\u2066-\u2069]/
      ...
      # Section 4 says the property values are the ones RFC 7591 registers,
      # but not that RFC 7591's defaults come with them — and that default,
      # client_secret_basic, is a method Section 4.1 forbids these clients
      # outright, so it cannot be the reading the draft intends. What is
      # assumed instead is the one method every such client can use.
      #
      # Refusing an omitted value is the other way to read the silence, and the
      # working group has not settled which is right: the two are the options
      # weighed in draft issue #92, "Provide guidance on omission of
      # token_endpoint_auth_method", which is open. What is assumed here is
      # therefore provisional, and follows the clients this feature exists to
      # serve: the AT Protocol OAuth profile, the deployed ecosystem the draft
      # comes from, makes the property optional and asks it only of
      # confidential clients ("confidential clients must set this to
      # private_key_jwt"), so a public client omitting it is the ordinary case
      # there — refusing it would turn every one of those documents into an
      # invalid_client. Revisit when the draft says which it wants; a document
      # that names the method explicitly is unaffected either way.
      #
      # An explicit null counts as absent, the way it does for every other
      # optional member here: a client that serializes its whole metadata
      # object with nulls for what it did not set describes the same document
      # as one that omitted them. Written as an explicit nil? rather than
      # `||`, so a non-nil value of the wrong type still reaches the type
      # check in validate_token_endpoint_auth_method!.
      def token_endpoint_auth_method
        value = attributes["token_endpoint_auth_method"]
        value.nil? ? PUBLIC_AUTH_METHOD : value
      end
      ...
      # Judged on the value rather than on the property's presence, the way
      # token_endpoint_auth_method is: a client that serializes its whole
      # metadata object with nulls for what it did not set has not used the
      # properties Section 4.1 forbids, it has only spelled out their
      # absence.
      def validate_no_forbidden_properties!
        present = FORBIDDEN_PROPERTIES.reject { |property| attributes[property].nil? }
      ...
      def validate_client_name!
        ...
        raise ValidationError, "client_name is not valid UTF-8 or contains a NUL" unless valid_utf8?(name)

        if name.match?(UNSAFE_DISPLAY_CHARACTERS)
          raise ValidationError, "client_name contains control or bidirectional formatting characters"
        end
      ...
      def validate_token_endpoint_auth_method!
        ...
        configured = configured_auth_methods(method)
        ...
        return if configured.none?(&:uses_shared_secret?)
      ...
      # never authenticate.
      # Every configured strategy declaring the name, not the first of them:
      # two may declare one name (a host renaming a method while keeping the
      # old registration key working), and what a document is held to is what
      # can actually authenticate it. Server stamps the declared name of
      # whichever strategy matched the request, so any of them satisfies the
      # check Client.authenticate makes — a document naming a method one
      # secret-free strategy declares would otherwise also be satisfied by a
      # secret-based one declaring the same name.
      def configured_auth_methods(name)
        Doorkeeper.config.client_authentication_methods.select do |method|
          method.auth_method_name == name
        end
      end

      # A document may legitimately omit redirect_uris (a client using only
      # grants that never redirect), and the row it materializes then stores
      # none — RedirectUriValidator permits that for materialized rows even
      # where registered applications must have one. Section 4.2 requires
      # registered redirect URIs for the grants that do redirect, which the
      # authorization endpoint's exact-match check enforces: nothing ever
      # matches an empty registration, so such a client cannot start a
      # redirect-based flow but keeps the ones that never redirect.
      def validate_redirect_uris!
        ...
        unless uris.all? { |uri| valid_utf8?(uri) }
          raise ValidationError, "redirect_uris contains a URI that is not valid UTF-8 or contains a NUL"
        end

        return if uris.all? { |uri| single_uri?(uri) }

        raise ValidationError, "redirect_uris entries must each hold a single URI"
      end

      # One entry, one URI (RFC 7591 Section 2). The column stores them
      # newline-separated and every reader of it splits on whitespace, so an
      # entry holding two URIs separated by a space would register both, and
      # a blank one would register nothing while looking like a
      # registration. A URI carries no whitespace of its own (RFC 3986), so
      # refusing the entry costs a well-formed document nothing.
      def single_uri?(uri)
        uri.present? && !uri.match?(/\s/)
      end
      ...
      def validate_jwks!
        ...
        set = attributes["jwks"]
        # Compared on the value, not on the key: a document carrying
        # `"jwks": null` beside a real jwks_uri publishes exactly one key set,
        # and the rest of this method already reads that jwks as absent.
        raise ValidationError, "jwks and jwks_uri must not both be present" if !uri.nil? && !set.nil?
      ...
      # The key fetcher accepts nothing but an https URL with a host and a
      # port a TCP connection could be made to (as does the metadata document
      # fetch itself, draft Section 3; URI.parse itself accepts any integer
      # as a port), so a jwks_uri failing this is one no key will ever be
      # read from — statically, not because a server happens to be down.
      # Rejecting it here keeps such a document out of the cache instead of
      # letting it stand until an assertion is refused with a bare
      # invalid_client.
      def fetchable_jwks_uri?(value)
        parsed = URI.parse(value)
        parsed.is_a?(URI::HTTPS) && parsed.host.present? && UrlValidator::PORT_RANGE.cover?(parsed.port)
