...
module Doorkeeper
  module ClientAuthentication
    Credentials = Struct.new(:uid, :secret) do
      ...
      #
      # Doorkeeper::Server stamps it from the strategy it selected — always,
      # overwriting any value that strategy set for itself — so a caller that
      # must know *how* a client authenticated depends on the server's record
      # rather than on each strategy's own word. A strategy that declares no
      # IANA name leaves this nil, which such a caller has to treat as
      # "unknown".
