...
module Doorkeeper
  class Server
    ...
    def credentials
      ...
      # client authenticated reads it from the credentials rather than
      # trusting each strategy to volunteer it. The selected strategy's
      # declared name always wins, including when it declares none and this
      # stays nil: a strategy that labelled the credentials with a *different*
      # method's name would otherwise decide on its own word which clients it
      # is allowed to authenticate, which is exactly what reading the name off
      # the selected strategy prevents. A frozen value object a host strategy
      # hands back is copied rather than skipped: skipping would leave the
      # strategy's own label standing, which is exactly the word this stamp
      # exists not to take. `dup` answers an unfrozen copy and carries the
      # struct's members and ivars with it.
