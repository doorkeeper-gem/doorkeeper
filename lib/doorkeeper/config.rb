...
module Doorkeeper
  ...
  class Config
    ...
    class Builder < AbstractBuilder
      ...
      # secret in the `old_secret` column and the token endpoint keeps
      # accepting it, so a client can be updated without a window in which its
      # credentials are rejected. The grace period ends when the application
      # calls `#clear_old_secret!` — Doorkeeper expires nothing on its own.
      #
    ...
    # How long a superseded client secret keeps authenticating, as a duration.
